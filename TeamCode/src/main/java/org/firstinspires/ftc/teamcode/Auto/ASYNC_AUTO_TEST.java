...
public class ASYNC_AUTO_TEST extends LinearOpMode {
    ...
    double amountIncreaseWarehouseX = 1.5;
            ...
            switch (state) {
                ...
                case INTAKE:
                    ...
                    if(((DistanceSensor) Oscar.colorBack).getDistance(DistanceUnit.CM) < 1.5) {
                        ...
                        Oscar.drive.setPoseEstimate(new Pose2d(Oscar.drive.getPoseEstimate().getX(), Oscar.drive.getPoseEstimate().getY() + 1, Oscar.drive.getPoseEstimate().getHeading()));
                        state = STATE.FORWARD;
                        time.reset();
                    }
                    break;
